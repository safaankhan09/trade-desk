...
const futuresCheckGrid = document.getElementById("futuresCheckGrid");
const timeframeButtons = Array.from(document.querySelectorAll("[data-timeframe]"));
const simSelectedSymbol = document.getElementById("simSelectedSymbol");
const simSelectedTimeframe = document.getElementById("simSelectedTimeframe");
const simMarketPrice = document.getElementById("simMarketPrice");
const simulatorForm = document.getElementById("simulatorForm");
const simDirectionInput = document.getElementById("simDirection");
const simEntryPriceInput = document.getElementById("simEntryPrice");
const simSizeInput = document.getElementById("simSize");
const simulatorMeta = document.getElementById("simulatorMeta");
const simulatorPositions = document.getElementById("simulatorPositions");
...
const alertLevelInput = document.getElementById("alertLevel");
const alertLeadMinutesInput = document.getElementById("alertLeadMinutes");
const alertRiskTargetInput = document.getElementById("alertRiskTarget");
...
const ALERTS_STORAGE_KEY = "kts-trading-alerts";
const PAPER_TRADES_STORAGE_KEY = "kts-paper-trades";
let observedRiskStatus = null;
let currentTimeframe = "3m";

const FUTURES_EXPECTATIONS = {
  NQ: { min: 10000, max: 30000, name: "Nasdaq 100 E-mini Futures" },
  ES: { min: 3000, max: 8000, name: "S&P 500 E-mini Futures" },
  GC: { min: 1500, max: 5000, name: "Gold Futures" },
  CL: { min: 20, max: 150, name: "Crude Oil Futures" },
};

const SESSION_WINDOWS = [
  { id: "asia", name: "Asia Session", startHour: 19, endHour: 4, note: "Quieter index flow, cleaner FX rotation, and thinner liquidity for US index futures." },
  { id: "london", name: "London Session", startHour: 3, endHour: 8, note: "Liquidity expands quickly as Europe comes online. Good for FX momentum and macro follow-through." },
  { id: "newyork", name: "New York Session", startHour: 8, endHour: 17, note: "Highest participation for US products, strongest opening volatility, and the best read on index futures." },
];
...
function renderTradingViewWidget(instrumentId = selectedInstrumentId) {
  ...
  new window.TradingView.widget({
    ...
    interval: TIMEFRAME_TO_TRADINGVIEW[currentTimeframe] || "3",
...
function loadPaperTrades() {
  try {
    return JSON.parse(localStorage.getItem(PAPER_TRADES_STORAGE_KEY) || "[]");
  } catch {
    return [];
  }
}
...
function savePaperTrades(trades) {
  localStorage.setItem(PAPER_TRADES_STORAGE_KEY, JSON.stringify(trades));
}

function getDefaultTimeframeForInstrument(instrumentId = selectedInstrumentId) {
  const groupName = Object.entries(MARKET_GROUPS).find(([, ids]) => ids.includes(instrumentId))?.[0] || activeGroup;
  return groupName === "stocks" ? "1D" : "3m";
}

function getMarketPriceForInstrument(instrumentId = selectedInstrumentId) {
  const marketItem = findMarketItemById(instrumentId);
  if (marketItem && Number.isFinite(Number(marketItem.price)) && Number(marketItem.price) > 0) {
    return Number(marketItem.price);
  }
  if (latestChartPayload?.id === instrumentId && Number.isFinite(Number(latestChartPayload.latestPrice))) {
    return Number(latestChartPayload.latestPrice);
  }
  return null;
}
...
function renderTimeframeButtons() {
  timeframeButtons.forEach((button) => {
    button.classList.toggle("active", button.dataset.timeframe === currentTimeframe);
  });
}

function findMarketItemById(id) {
  for (const payload of Object.values(latestBoardsByGroup)) {
    const match = payload?.items?.find((item) => item.id === id);
    if (match) return match;
  }
  return latestBoard.find((item) => item.id === id) || null;
}
...
function getCurrentSessionState() {
  ...
  const isInWindow = (sessionHourStart, sessionHourEnd) => {
    if (sessionHourStart < sessionHourEnd) {
      return hour >= sessionHourStart && hour < sessionHourEnd;
    }
    return hour >= sessionHourStart || hour < sessionHourEnd;
  };

  const active = SESSION_WINDOWS.find((session) => isInWindow(session.startHour, session.endHour));
  const ranked = SESSION_WINDOWS
    .map((session) => {
      let delta = session.startHour - hour;
      if (delta <= 0) delta += 24;
      return { session, delta };
    })
    .sort((a, b) => a.delta - b.delta);

  const nextSession = ranked[0].session;
  const nextStart = new Date(now);
  nextStart.setMinutes(0, 0, 0);
  nextStart.setHours(nextSession.startHour);
  if (nextSession.startHour <= hour) {
...
function renderSessionPlanner() {
  ...
  activeSessionName.textContent = active?.name || "Between Major Sessions";
  nextSessionName.textContent = state.next?.name || "Asia Session";
  sessionCountdown.textContent = formatDuration(state.countdownMs);
  sessionNote.textContent = active?.note || "This is a transition window between core sessions. Expect thinner liquidity and less reliable follow-through.";
  sessionPill.className = `signal-pill ${active ? "safe" : "neutral"}`;
  sessionPill.textContent = active ? "Active" : "Transition";

  sessionCards.innerHTML = SESSION_WINDOWS
    .map((session) => {
      ...
      return `
        <article class="session-card ${isActive ? "active" : ""}">
          <div class="session-card-top">
            ...
            <span class="session-state">${isActive ? "Live" : "Standby"}</span>
          </div>
          <div class="session-hours">${formatHourLabel(session.startHour)} - ${formatHourLabel(session.endHour)}</div>
          <div class="session-copy">${session.note}</div>
        </article>
      `;
    })
    .join("");
...
function renderSimulator() {
  const openTrades = loadPaperTrades();
  const marketPrice = getMarketPriceForInstrument(selectedInstrumentId);
  simSelectedSymbol.textContent = selectedInstrumentId;
  simSelectedTimeframe.textContent = currentTimeframe;
  simMarketPrice.textContent = marketPrice != null ? formatNumber(marketPrice) : "Unavailable";

  if (document.activeElement !== simEntryPriceInput && marketPrice != null) {
    simEntryPriceInput.value = String(marketPrice);
  }

  simulatorMeta.textContent = marketPrice != null
    ? "PnL updates from the latest cached market-board or chart payload, not just the TradingView widget."
    : "Waiting for a valid cached market price before opening or marking positions.";

  if (!openTrades.length) {
    simulatorPositions.innerHTML = `<p class="muted">No paper trades open yet.</p>`;
    return;
  }

  simulatorPositions.innerHTML = openTrades
    .slice()
    .reverse()
    .map((trade) => {
      const currentPrice = getMarketPriceForInstrument(trade.symbol);
      ...
      return `
        <article class="sim-position-card ${trade.symbol === selectedInstrumentId ? "active" : ""}">
          <div class="sim-position-top">
            <div>
              <div class="sim-position-symbol">${trade.symbol} • ${trade.timeframe}</div>
              <div class="sim-position-meta">${trade.direction} • size ${formatNumber(trade.size)} • opened ${trade.openedAt}</div>
            </div>
            <button class="ghost-button sim-close-button" type="button" data-close-trade="${trade.id}">Close</button>
          </div>
          <div class="sim-position-grid">
            <div><span>Entry</span><strong>${formatNumber(trade.entryPrice)}</strong></div>
            <div><span>Current</span><strong>${currentPrice != null ? formatNumber(currentPrice) : "Unavailable"}</strong></div>
            <div><span>PnL</span><strong class="${pnl != null && pnl >= 0 ? "positive" : "negative"}">${pnl != null ? formatSigned(pnl) : "Unavailable"}</strong></div>
          </div>
        </article>
      `;
    })
  ...
  simulatorPositions.querySelectorAll("[data-close-trade]").forEach((button) => {
    button.addEventListener("click", () => closePaperTrade(button.dataset.closeTrade));
  });
}

function futuresCheckRowsForItem(item, boardStatus = "cached") {
  const expectation = FUTURES_EXPECTATIONS[item.id];
  const tradingViewSymbol = tradingViewSymbolForInstrument(item.id);
  const sourceType = item.sourceType || item.kind || "unknown";
  const usingRealFutures = sourceType === "future";
  const realisticPrice = usingRealFutures && expectation
    ? Number(item.price) >= expectation.min && Number(item.price) <= expectation.max
    : false;
  const delayed = boardStatus !== "live" || sourceType !== "future";
  const backendMappingCorrect = usingRealFutures || sourceType === "index_fallback" || sourceType === "proxy";
  const mappingLabel = usingRealFutures
    ? "Real futures feed"
  ...
  return `
    <article class="futures-check-card">
      <div class="futures-check-top">
        <div>
          <div class="futures-check-symbol">${item.id}</div>
          <div class="futures-check-name">${expectation?.name || item.description}</div>
        </div>
        <span class="signal-pill ${usingRealFutures ? "safe" : "caution"}">${usingRealFutures ? "Real" : "Fallback"}</span>
      </div>
      <div class="futures-check-grid-inner">
        <div><span>Data Type</span><strong>${usingRealFutures ? "Real futures data" : "Proxy data"}</strong></div>
        <div><span>Current Symbol</span><strong>${escapeHtml(item.sourceSymbol || item.symbol || "-")}</strong></div>
        <div><span>TradingView</span><strong>${escapeHtml(tradingViewSymbol)}</strong></div>
        <div><span>Price Scale</span><strong>${realisticPrice ? "Realistic" : "Not contract-realistic"}</strong></div>
        <div><span>Delay Risk</span><strong>${delayed ? "May be delayed" : "Fresh live mapping"}</strong></div>
        <div><span>Backend Mapping</span><strong>${backendMappingCorrect ? mappingLabel : "Needs review"}</strong></div>
      </div>
      <p class="muted futures-check-note">
        ${usingRealFutures ? "The app is using a futures-first mapping for this contract." : (item.sourceNote || "Real futures pricing is not available on the current plan, so the app is using clearly labeled fallback data.")}
      </p>
    </article>
  `;
}

function renderFuturesCheck() {
  const futuresPayload = latestBoardsByGroup.futures || null;
  const futuresBoard = futuresPayload?.items || [];
  if (!futuresBoard.length) {
    futuresCheckGrid.innerHTML = `<p class="muted">Waiting for futures board data...</p>`;
    return;
  }

  futuresCheckGrid.innerHTML = futuresBoard
    .filter((item) => FUTURES_EXPECTATIONS[item.id])
    .map((item) => futuresCheckRowsForItem(item, futuresPayload?.cacheStatus || "cached"))
    .join("");
}

function renderAlerts(alerts) {
  if (!alerts.length) {
    alertsList.innerHTML = `<p class="muted">No alerts configured yet.</p>`;
    return;
  }
    ...
    .map((alert) => `
      <article class="alert-card ${alert.triggered ? "triggered" : ""}">
        <div class="alert-card-top">
          ...
          <span class="signal-pill ${alert.triggered ? "caution" : "neutral"}">${alert.triggered ? "Triggered" : "Armed"}</span>
        </div>
        <div class="alert-card-meta">${escapeHtml(alert.description)}</div>
        <div class="alert-card-meta">${alert.lastTriggeredAt ? `Last hit: ${alert.lastTriggeredAt}` : "Waiting for trigger."}</div>
      </article>
    `)
    .join("");
}
...
function evaluateAlerts() {
  ...
  const now = new Date();
  const riskStatus = latestCalendarPayload?.assessment?.status || null;
  const vix = latestCalendarPayload?.vix || null;
  ...
  const updated = alerts.map((alert) => {
    ...
    if (alert.type === "price_above") {
      const item = findMarketItemById(alert.symbol);
      triggered = item ? Number(item.price) >= Number(alert.level) : false;
    } else if (alert.type === "price_below") {
      const item = findMarketItemById(alert.symbol);
      triggered = item ? Number(item.price) <= Number(alert.level) : false;
    } else if (alert.type === "news_event") {
      triggered = events.some((event) => {
        if (!event.datetimeLocal) return false;
        const deltaMs = new Date(event.datetimeLocal).getTime() - now.getTime();
        return deltaMs >= 0 && deltaMs <= Number(alert.leadMinutes || 0) * 60000;
      });
    } else if (alert.type === "volatility_spike") {
      triggered = vix ? Number(vix.price) >= Number(alert.level) : false;
    } else if (alert.type === "risk_status_change") {
      triggered = Boolean(riskStatus && observedRiskStatus && riskStatus !== observedRiskStatus && riskStatus === alert.targetStatus);
    }

    if (triggered !== Boolean(alert.triggered)) {
      changed = true;
    }
    ...
    return {
      ...alert,
      triggered,
      lastTriggeredAt: triggered && !alert.triggered ? now.toLocaleString() : alert.lastTriggeredAt || "",
    };
  });

  if (riskStatus) {
    observedRiskStatus = riskStatus;
  }
...
function renderBoard(items, payload) {
  ...
  watchlistEl.querySelectorAll("[data-id]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      currentTimeframe = getDefaultTimeframeForInstrument(selectedInstrumentId);
...
function buildAlertLabel(type, values) {
  if (type === "price_above") return `${values.symbol} above ${values.level}`;
  if (type === "price_below") return `${values.symbol} below ${values.level}`;
  if (type === "news_event") return `News event within ${values.leadMinutes} min`;
  if (type === "volatility_spike") return `VIX above ${values.level}`;
  return `Risk status becomes ${values.targetStatus}`;
}

function buildAlertDescription(type, values) {
  if (type === "price_above") return `Triggers when ${values.symbol} trades above ${values.level}.`;
  if (type === "price_below") return `Triggers when ${values.symbol} trades below ${values.level}.`;
  if (type === "news_event") return `Triggers when a scheduled report is within ${values.leadMinutes} minutes.`;
  if (type === "volatility_spike") return `Triggers when the volatility input rises above ${values.level}.`;
  return `Triggers when the risk desk status changes into ${values.targetStatus}.`;
}
...
function closePaperTrade(tradeId) {
  ...
  if (currentPrice == null) {
    simulatorMeta.textContent = "A cached market price is required before a paper trade can be closed.";
    return;
  }
  ...
  addJournalEntry({
    ...
    pnl,
    setup: "Paper trade",
    notes: `Closed from paper simulator at ${new Date().toLocaleString()}.`,
  });

  savePaperTrades(trades.filter((item) => item.id !== tradeId));
  renderSimulator();
}

function renderJournal(entries) {
  if (!entries.length) {
    journalTableBody.innerHTML = `<tr><td colspan="8" class="muted">No trades logged yet.</td></tr>`;
  } else {
...
timeframeButtons.forEach((button) => {
  button.addEventListener("click", () => {
    currentTimeframe = button.dataset.timeframe || currentTimeframe;
    renderTimeframeButtons();
    renderTradingViewWidget(selectedInstrumentId);
    renderSimulator();
  });
});
...
simulatorForm.addEventListener("submit", (event) => {
  event.preventDefault();
  const entryPrice = Number(simEntryPriceInput.value);
  const size = Number(simSizeInput.value);
  if (!Number.isFinite(entryPrice) || entryPrice <= 0 || !Number.isFinite(size) || size <= 0) {
    simulatorMeta.textContent = "Use a valid entry price and size before opening a paper trade.";
    return;
  }
  ...
  trades.push({
    ...
    entryPrice,
    direction: simDirectionInput.value,
    size,
    openedAt: new Date().toLocaleString(),
    openedAtIso: new Date().toISOString(),
  });
  savePaperTrades(trades);
  simulatorMeta.textContent = `${selectedInstrumentId} ${currentTimeframe} paper trade opened and now tracking against cached market data.`;
  renderSimulator();
  simulatorForm.reset();
  const marketPrice = getMarketPriceForInstrument(selectedInstrumentId);
  if (marketPrice != null) {
...
alertsForm.addEventListener("submit", (event) => {
  ...
  const values = {
    symbol: alertSymbolInput.value,
    level: alertLevelInput.value,
    leadMinutes: alertLeadMinutesInput.value,
    targetStatus: alertRiskTargetInput.value,
  };
  ...
  alerts.push({
    ...
    label: buildAlertLabel(type, values),
    description: buildAlertDescription(type, values),
    triggered: false,
    lastTriggeredAt: "",
  });
  saveAlerts(alerts);
