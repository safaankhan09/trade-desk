...
function renderBoard(items, payload) {
  ...
  stocksMeta.textContent = `${MARKET_GROUP_LABELS[activeGroup] || activeGroup} board ${payload.cacheStatus || "live"} • ${formatAgeLabel(payload.cacheAgeSeconds)} • ${payload.sourceNote || ""}`;
}
...
function renderChart(payload) {
  ...
  chartSource.textContent = `${payload.sourceNote || ""} ${payload.cacheStatus ? `Status: ${payload.cacheStatus}.` : ""}`.trim();
}
...
async function loadBoard(group = activeGroup, forceRefresh = false) {
  ...
  try {
    ...
    stocksMeta.textContent = `Failed to load ${group} board.`;
  }
}
